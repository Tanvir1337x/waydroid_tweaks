...
def resize(img_file, size):
    run(
        ["sudo", "e2fsck", "-y", "-f", img_file], ignore=r"^e2fsck \d+\.\d+\.\d (.+)\n$"
    )
    run(
        ["sudo", "resize2fs", img_file, size], ignore=r"^resize2fs \d+\.\d+\.\d (.+)\n$"
    )
...
def get_image_dir():
    ...
    if "waydroid" not in cfg:
        Logger.error(
            "Required entry in config was not found, Cannot continue!"
        )  # magisk
        sys.exit(1)
