        ...
        for lz_file in os.listdir(os.path.join(self.extract_to, "Core")):
            ...
            if lz_file not in self.skip:
                if lz_file not in self.non_apks:
                    ...
                    xx_dpi = os.listdir(
                        os.path.join(self.extract_to, "appunpack", app_name)
                    )[0]
                    app_priv = os.listdir(
                        os.path.join(self.extract_to, "appunpack", app_name, "nodpi")
                    )[0]
                    ...
                    for app in os.listdir(app_src_dir):
                        shutil.copytree(
                            ...
                            os.path.join(
                                self.copy_dir, self.partition, "priv-app", app
                            ),
                            dirs_exist_ok=True,
                        )
                        for f in os.listdir(
                            os.path.join(self.copy_dir, self.partition, "priv-app", app)
                        ):
                    ...
                    common_content_dirs = os.listdir(
                        os.path.join(self.extract_to, "appunpack", app_name, "common")
                    )
                    for ccdir in common_content_dirs:
                        shutil.copytree(
                            os.path.join(
                                self.extract_to, "appunpack", app_name, "common", ccdir
                            ),
                            os.path.join(self.copy_dir, self.partition, ccdir),
                            dirs_exist_ok=True,
                        )
